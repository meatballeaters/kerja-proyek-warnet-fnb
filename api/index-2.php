...
?>
    ...
    <main class="max-w-7xl mx-auto px-6 pt-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-900/80 p-4 rounded-2xl border border-slate-800">
            <div class="relative w-full md:w-96">
                ...
                <input type="text" id="search-minuman" onkeyup="renderDrinkProducts()" placeholder="Cari Es Teh, Kopi Susu, Matcha, Boba..." 
                    class="w-full bg-slate-950 text-slate-100 placeholder-slate-500 text-xs pl-10 pr-4 py-2.5 rounded-xl border border-slate-800 focus:outline-none focus:border-cyan-500 transition-all">
            </div>

            <div class="flex items-center gap-2 overflow-x-auto hide-scrollbar w-full md:w-auto">
                <button onclick="selectDrinkCategory('all')" class="drink-cat-btn active px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap bg-cyan-600 text-white shadow-md shadow-cyan-600/30" data-cat="all">
                    🥤 Semua Minuman
                </button>
                <?php foreach ($drink_categories as $cat): ?>
                <button onclick="selectDrinkCategory('<?= htmlspecialchars($cat['slug']) ?>')" class="drink-cat-btn px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap bg-slate-950 text-slate-400 border border-slate-800 hover:text-white transition-all" data-cat="<?= htmlspecialchars($cat['slug']) ?>">
        ...
        <div id="drink-product-grid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            <!-- Injected via JS -->
        </div>
    ...
    <div id="floating-cart" class="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-xl px-4 z-40 transition-all transform translate-y-28 opacity-0 pointer-events-none">
        <div class="bg-slate-900/95 backdrop-blur-md border border-cyan-500/40 p-3.5 rounded-2xl shadow-2xl flex items-center justify-between gap-4">
            <div class="flex items-center gap-3 pl-2">
                <div class="relative bg-cyan-600/20 text-cyan-400 p-2.5 rounded-xl border border-cyan-500/30">
                    <i class="fa-solid fa-basket-shopping text-base"></i>
                    <span id="cart-count-badge" class="absolute -top-1.5 -right-1.5 bg-cyan-500 text-slate-950 font-extrabold text-[10px] w-4 h-4 rounded-full flex items-center justify-center shadow-md">0</span>
                </div>
                <div>
            ...
            <button onclick="openCartModal()" class="bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-600 hover:to-indigo-700 text-white font-bold text-xs px-5 py-2.5 rounded-xl shadow-lg flex items-center gap-2 active:scale-95 transition-transform">
    ...
    <div id="detail-modal" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 opacity-0 pointer-events-none transition-all duration-300">
        <div class="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl transform scale-95 transition-transform duration-300" id="detail-modal-card">
            <div class="relative h-56 bg-slate-800">
                <img id="modal-img" src="" alt="Detail Minuman" class="w-full h-full object-cover">
                ...
                <span id="modal-cat" class="absolute bottom-3 left-3 bg-cyan-500 text-slate-950 font-extrabold text-[10px] px-2.5 py-1 rounded-lg uppercase"></span>
            </div>
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center justify-between">
                        <span>Catatan Khusus Barista:</span>
                        <span class="text-[10px] text-slate-500 lowercase font-normal">(opsional)</span>
                    </label>
                    <textarea id="modal-notes" rows="2" placeholder="Contoh: Manisnya sedikit saja (less sugar), es batu dikit, jangan pakai sedotan..." class="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500 resize-none"></textarea>
                </div>
                ...
                <div class="flex items-center justify-between gap-4 pt-2 border-t border-slate-800">
                    ...
                    <button onclick="confirmAddToCart()" class="flex-1 bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-600 hover:to-indigo-700 text-white font-extrabold text-xs py-3 rounded-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
    ...
    <script>
        ...
        let selectedVariant = 'Dingin (Normal Ice)';
        ...
        function renderDrinkProducts() {
            const grid = document.getElementById('drink-product-grid');
            if (!grid) return;
            const searchInput = document.getElementById('search-minuman');
            const search = searchInput ? searchInput.value.toLowerCase().trim() : '';
            grid.innerHTML = '';

            const filtered = DRINK_PRODUCTS.filter(p => {
            ...
            if (filtered.length === 0) {
                grid.innerHTML = `
                    <div class="col-span-full py-12 text-center text-slate-500">
                        <i class="fa-solid fa-wine-glass text-3xl mb-2 block text-slate-600"></i>
                        <p class="text-xs font-bold text-slate-400">Minuman tidak ditemukan</p>
                    </div>
                `;
                return;
            }
            ...
            filtered.forEach(p => {
                ...
                card.className = "bg-slate-900 border border-slate-800/90 rounded-2xl overflow-hidden hover:border-cyan-500/40 transition-all flex flex-col justify-between shadow-lg group";
                card.innerHTML = `
                    <div>
                    ...
                    <div class="p-3 pt-0 flex items-center justify-between gap-1 mt-1">
                        ...
                        <button onclick="openDetailModal(${p.id})" class="bg-cyan-600/20 hover:bg-cyan-600 text-cyan-300 hover:text-white border border-cyan-500/30 text-[10px] font-bold px-2.5 py-1.5 rounded-xl flex items-center gap-1 transition-all active:scale-95">
        ...
        function selectDrinkCategory(cat) {
            activeCategory = cat;
            document.querySelectorAll('.drink-cat-btn').forEach(btn => {
                if (btn.dataset.cat === cat) {
                    btn.className = "drink-cat-btn active px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap bg-cyan-600 text-white shadow-md shadow-cyan-600/30";
                } else {
                    btn.className = "drink-cat-btn px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap bg-slate-950 text-slate-400 border border-slate-800 hover:text-white transition-all";
                }
            });
            renderDrinkProducts();
        }
        ...
        function selectVariant(btn, variantName) {
            ...
            btn.className = "variant-btn active bg-cyan-600 border border-cyan-500 text-white text-[11px] font-bold py-2 rounded-xl transition";
        }
        ...
        function renderCartModalList() {
            ...
            cartItems.forEach((item, index) => {
                ...
                const notesHtml = item.notes && item.notes.trim() !== ''
                    ? `<p class="text-[10px] text-cyan-300 italic bg-slate-950/80 px-2 py-0.5 rounded border border-slate-800 mt-1">"${item.notes}"</p>`
                    : '';
